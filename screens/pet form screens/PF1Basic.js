...
const locations = ['Johor', 'Kedah', 'Kelantan', 'Melaka', 'Negeri Sembilan', 'Pahang',
...
const PF1Basic = ({ navigation }) => {
  ...
  const renderBox = (animal, index) => {
    return (
      <TouchableOpacity
        key={index}
        style={[
          styles.box, species === animal.title && styles.boxPressed]}
        onPress={() => setSpecies(animal.title)}>
  ...
  const handleSubmit = () => {
    ...
    if (!name || (ageYear == 0 && ageMonth == 0) || !species)
      return;
        ...
        <View style={styles.container}>
          ...
          <View style={[styles.rowContainer, { justifyContent: 'space-between' }]}>
            <View style={styles.ageRowContainer}>
              ...
              <NumericInput
                value={ageYear}
                onChange={value => setAgeYear(value)}
                maxValue={99}
                minValue={0}
                rounded
                totalWidth={scale(100)}
                totalHeight={verticalScale(35)}
              />
            </View>
            <View style={styles.ageRowContainer}>
              ...
              <NumericInput
                value={ageMonth}
                onChange={value => setAgeMonth(value)}
                maxValue={99}
                minValue={0}
                rounded
                totalWidth={scale(100)}
          ...
          {chosenMedia && chosenMedia.length > 0 &&
            <FlatList
              ...
              renderItem={({ item, index }) => {
                return <View style={styles.cameraPics} >
                  <Image
                    source={{ uri: item }}
                    style={styles.cameraPics}
                  />
                  <Icon name="close-circle-outline" type="material-community" size={moderateScale(30)} color="white" containerStyle={{ position: 'absolute', top: 0, right: 0 }} />
                  <Icon name="close-circle" type="material-community" size={moderateScale(30)} containerStyle={{ position: 'absolute', top: 0, right: 0 }} onPress={() => handleRemoveChosenMedia(item)} />
                </View>;
              }}
            />
          }
          {(chosenMedia && chosenMedia.length > 0) ?
            <MediaPicker setChosenMedia={setChosenMedia} buttons />
            : <MediaPicker setChosenMedia={setChosenMedia} />}
        </View>
